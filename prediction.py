...
def make_spline_dataset(data_input, scalefactor_input, scalefactor_output, input_size, rowsize):
    ...
    for idx in range(len(rowsize)-1):
        if rowsize[idx+1] - rowsize[idx] <= 2:  continue
...
def hermite(hermite_y, hermite_dydx, hermite_x, input_x, rvir_standardize):
    ...
    for y, dydx, x, ip_x, rvir in zip(hermite_y, hermite_dydx, hermite_x, input_x, rvir_standardize):
        ## Convert verocity's unit from [km/s] to [Mpc/Gyr].
        ## 1Mpc = 3.086 * 10^22[m]
        ## 1Gyr = 3.156 * 10^16[s]
        v       = dydx * 3.156 / (3.086 * 10**6)
        func    = ip.CubicHermiteSpline(x, y, v)
...
if __name__ == "__main__":
    ...
    for direc in range(1, len(directory_list)):
    ...
    if "x" in LP.use_param_input and "vx" in LP.use_param_input and "hermite" not in methods:
        methods += ["hermite"]
    if "y" in LP.use_param_input and "vy" in LP.use_param_input and "hermite" not in methods:
        methods += ["hermite"]
    if "z" in LP.use_param_input and "vz" in LP.use_param_input and "hermite" not in methods:
        methods += ["hermite"]
    ...
    if "hermite" in methods:
        print("Calculate Giga-Year from ScaleFactor...")
    ...
    for p_key in LP.use_param_output:
        ...
        for met_key in methods:
            ...
            if met_key == "hermite":
                if not p_key in ["x", "y", "z"] and not "v"+p_key in ["vx", "vy", "vz"]:
                    continue
                else:
